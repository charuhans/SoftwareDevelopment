...
public class VenkatsPieceOfMindLogicTest {
  ...
  int[] allBlack = new int[] {0, 0, 0, 0, 0, 0};
  int[] allSilver = new int[] {7, 7, 7, 7, 7, 7};
  ...
  @Test public void testMatchGuessWithAllOneColor() {
    ...
    assertArrayEquals(new int[] {0, -1, -1, -1, -1, -1}, result);    
  }
  ...
  @Test public void testMatchGuessOnePositionWithDuplicatedTarget() {
    int[] result = matchColors(duplicatedTarget, 
      new int[] {1, 1, 6, 6, 6, 6});
    assertArrayEquals(new int[] {0, 7, -1, -1, -1, -1}, result);    
  }
  
  @Test public void testMatchGuessTwoPositionWithDuplicatedTarget() {
    int[] result = matchColors(duplicatedTarget, 
      new int[] {6, 1, 6, 6, 6, 1});
    assertArrayEquals(new int[] {0, 0, -1, -1, -1, -1}, result);    
  }
  ...
  List<Colors> colors = Arrays.asList(Colors.WHITE, Colors.RED, Colors.GREEN, Colors.BLUE, Colors.ORANGE, Colors.YELLOW, Colors.VIOLET, Colors.PINK);	
  
  List<Colors> scores = Arrays.asList(Colors.BLACK, null,  null,  null,  null,  null,  null, Colors.SILVER);
  ...
  public int[] matchColors(int[] targetColors, int[] guessColors) {
    ...
    {
        resultColors[i] = scores.indexOf(result.get(i));
    }
